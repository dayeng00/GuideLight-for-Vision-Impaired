...
def main():
    ...
    # results = model.train(data="datas/elevator_button/data.yaml", epochs=100, imgsz=640, device=0)
    ...
    # comet_ml.init(api_key="PrTNljI3CgerouhlK8pIVeS1j") # 已弃用
    comet_ml.login(api_key="PrTNljI3CgerouhlK8pIVeS1j")

    # 训练模型并启用 Comet 日志记录
    results = model.train(
        data=r"D:\bin\pycharm\GuideLight-for-Vision-Impaired\test\datasets\elevator_button\data.yaml",
        epochs=100,
        imgsz=640,
        batch=16,
