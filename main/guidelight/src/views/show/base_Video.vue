...
//   import { useUserStore } from "../../store/userStore";
  ...
  // 摄像头流地址
  const camera1Url = ref(`${serverUrl}/b_video/video_feed`);
  ...
  const startStreaming = () => {
    // TODO: 实现开启摄像头的逻辑
    // console.log("开启摄像头");
    camera1Url.value = `${serverUrl}/b_video/video_feed`;
 
    axios.post(`${serverUrl}/b_video/start_cameras`).then((response) => {
  ...
  const stopStreaming = () => {
    // TODO: 实现停止摄像头的逻辑
    // console.log("停止摄像头");
    axios.post(`${serverUrl}/b_video/stop_cameras`).then((response) => {
      console.log(response.data);
    })
  };
  </script>
